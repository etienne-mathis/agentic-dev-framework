...
# Issue #13: Scope case-insensitiv — Großbuchstaben im Scope dürfen nicht deadlocken.
RC=0; OUT=$(printf 'feat(useCart): add localStorage persistence (#34)\n' | bash "$CI/commitlint.sh") || RC=$?
assert_eq 0 "$RC" "gemischt-groß/klein Scope (useCart) gültig (Regression #13-Deadlock)"

RC=0; OUT=$(printf 'test(useCart): revise acceptance contract for #34 [AC-3]\n' | bash "$CI/commitlint.sh") || RC=$?
assert_eq 0 "$RC" "Contract-Commit mit CamelCase-Scope gültig (#13)"
